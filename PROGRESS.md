...
- [x] **Asteroids drifting on the raylib window** — `Asteroid` struct array, `initialize_asteroids` fills caller-owned array in place (void, not return-a-local), compound literals `(Vec2){...}`, symmetric `±` speed bounds via `static const float` so they go all directions. Real `dt` via `GetFrameTime()`. `DrawCircle` per asteroid.
